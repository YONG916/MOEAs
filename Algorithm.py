...
class MoeaAlgorithm(Algorithm):  # 多目标优化算法模板父类
    ...
    def logging(self, pop):
        ...
        """
        # 2023年11/11, 输入True Pareto Front
        # 2023年11/11, 判断是否输入PF
        """
        dict_para = self.problem.name['dict_para']
        if dict_para['Read_PF']:
            self.problem.ReferObjV = self.problem.name['PF']
        # print('NSGA2', type(NDSet.ObjV), type(self.problem.name['PF']))
        # print(self.problem.name['PF'].values)
        #
        if self.problem.ReferObjV is not None:
            GD = ea.indicator.GD(NDSet.ObjV, self.problem.ReferObjV)
            self.log['gd'].append(GD)  # 计算GD指标
            # print('当前支配度：', GD)
        ...
        # 2022/3/4
        # 记录当前可行解的数量
        self.log['numFeasible_sol'].append(len(NDSet.ObjV))
        ...
        # 目的：保存每一代的hv与spacing
        # Gen = len(self.log['hv'])
        dict = self.problem.name
        # 2023年11/13
        """
        File_name = 'Result\\Solution2\\Total_BSS\\Whole_factor\\pareto_result\\' + dict['algorithm_name']  + '\\Evaluation\\'
        if dict['save_csv']:
            EVs, BSSs,Bats = dict['EVs'], dict['BSSs'],dict['Bats']
            numEV, numBSS, numBat  = len(EVs), len(BSSs), int(BSSs[0].availB[0])
            if len(self.log['hv']) == dict['Gen']:
                if dict_para['Read_PF']:
                    # 1.HV
                    path = File_name + 'HV\\Gen' + str(dict['Gen']) + '_NP' + str(dict['NP']) + '_Repeats' + str(dict['repeat-th'] ) + '_BSS' + str(dict['BSS_j']) + '_EVs' + str(numEV) + '_BSSs' + str(numBSS) + '_Bats' + str(numBat) + '_' + dict['algorithm_name'] + '_ref' + str(0) + '_encode' + '.csv'
                    tmp = pd.DataFrame(self.log['hv'])
                    tmp.to_csv(path)

                    # 2.GD
                    path = File_name + 'GD\\Gen' + str(dict['Gen']) + '_NP' + str(dict['NP']) + '_Repeats' + str(dict['repeat-th'] ) + '_BSS' + str(dict['BSS_j']) + '_EVs' + str(numEV) + '_BSSs' + str(numBSS) + '_Bats' + str(numBat) + '_' + dict['algorithm_name'] + '_ref' + str(0) + '_encode' + '.csv'
                    tmp = pd.DataFrame(self.log['gd'])
                    tmp.to_csv(path)

                    # 3.IGD
                    path = File_name + 'IGD\\Gen' + str(dict['Gen']) + '_NP' + str(dict['NP']) + '_Repeats' + str(dict['repeat-th'] ) + '_BSS' + str(dict['BSS_j']) + '_EVs' + str(numEV) + '_BSSs' + str(numBSS) + '_Bats' + str(numBat) + '_' + dict['algorithm_name'] + '_ref' + str(0) + '_encode' + '.csv'
                    tmp = pd.DataFrame(self.log['igd'])
                    tmp.to_csv(path)

                # 4.numFeasible_sol
                path = File_name + 'numFeasible_sol\\Gen' + str(dict['Gen']) + '_NP' + str(dict['NP']) + '_Repeats' + str(dict['repeat-th'] ) + '_BSS' + str(dict['BSS_j']) + '_EVs' + str(numEV) + '_BSSs' + str(numBSS) + '_Bats' + str(numBat) + '_' + dict['algorithm_name'] + '_ref' + str(0) + '_encode' + '.csv'
                tmp = pd.DataFrame(self.log['numFeasible_sol'])
                tmp.to_csv(path)

                # 5.SP
                path = File_name + 'SP\\Gen' + str(dict['Gen']) + '_NP' + str(dict['NP']) + '_Repeats' + str(dict['repeat-th'] ) + '_BSS' + str(dict['BSS_j']) + '_EVs' + str(numEV) + '_BSSs' + str(numBSS) + '_Bats' + str(numBat) + '_' + dict['algorithm_name'] + '_ref' + str(0) + '_encode' + '.csv'
                tmp = pd.DataFrame(self.log['spacing'])
                tmp.to_csv(path)

# df = pd.DataFrame({'HV': self.log['hv'], 'Spacing': self.log['spacing']})
# path = 'Result\\Solution2\\Total_BSS\\Whole_factor\\pareto_result\\NSGA2\\Evaluation\\csv\\Gen' + str(dict['Gen']) + '_NP' + str(dict['NP'])  + '_Repeats' \
#        + str(dict['repeat-th']) + '_BSS' + str(dict['BSS_j'])  + '_EVs' + str(numEV) \
#        + '_BSSs' + str(numBSS)+ '_Bats' + str(numBat) + '_' + dict['algorithm_name']+ '_ref'  +  str(dict['ref']) + '.csv'
# df.to_csv(path)
